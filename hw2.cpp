...
void multiply2(float v[4], float m[16], float result[4])
{
    ...
    for (row = 0; row < 4; row++) {
        for (col = 0; col < 4; col++) {
            //printf("multiplying %f by %f in multiply2\n", v[col], m[(row*4)+col]);
            result[row] += v[col] * m[(col*4)+row];
            //printf("current result in multiply 2 is %f\n", result[row]);
        }
    }
}
...
void view_transform()
{
    //Float *prod = (float *) malloc(sizeof(float) * 4);
    // compute az, bz, cz -- direction of view vector
    float az = lx - av;
    float bz = ly - bv;
    float cz = lz - cv;
    //printf("az, bz, cz is <%f, %f, %f>\n", az, bz, cz);
...
void drawline(float *p1, float *p2)
{
    ...
    // clip
    if(!clip(prod1, prod2)) {
        //printf("drawing line from (%f, %f) to (%f, %f).\n", prod1[0], prod1[1], prod2[0], prod2[1]);
        glBegin(GL_LINES);
...
void setupViewport(int w, int h)
{
    ...
    glLoadIdentity();
    //gluOrtho2D(0.0, XSCALE*w/WIN_WIDTH, 0.0, h*YSCALE/WIN_HEIGHT );
...
int main(int argc, char *argv[])
{
    ...
    main_window = glutCreateWindow("HW2");
    ...
    new GLUI_StaticText(control_panel, "HW2");
